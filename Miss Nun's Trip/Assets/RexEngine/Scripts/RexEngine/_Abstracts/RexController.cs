...
namespace RexEngine
{
	public class RexController:MonoBehaviour
	{
		...
		public class Slots
		{
			...
			[Tooltip("If slotted, this RexAIRoutine will be active when this RexController becomes active.")]
			public RexAIRoutine startingAIRoutine;
		}
		...
		[Tooltip("Whether or not pressing in alternate directions causes the actor to face in that direction.")]
		public bool isTurningAllowed = true;
		...
		[Tooltip("Advanced Editor options for RexController.")]
		...
		protected RexState defaultState;
		protected RexState fallingState;
		protected DeathState deathState;
		protected MovingState movingState;
		protected LandingState landingState;
		protected bool canAttackThroughOverridingAnimation = false; //Whether or not the current overriding animation (if any) will allow attacks to perform while it's active
		...
		{ 
			...
			if(_state != currentState || canInterruptSelf)
			{
				...
				if(slots.audio && _state.audioClip && _state.id != KnockbackState.idString)
				{
		...
		public void EndAllStates() //Ends movements for every single attached RexState at once
		{
			...
			canAttackThroughOverridingAnimation = false;
		...
		public virtual void SetStateToDefault(bool canInterruptSelf = false, bool disallowFallingState = false, bool allowTransitionFromDeath = false) //Sets currentState either to DefaultState or to FallingState depending on whether or not it is grounded
		{
		...
		public virtual void FaceDirection(Direction.Horizontal _direction) //Called when the Controller is moved left or right, and thus might change direction; calls Turn() if we did change
		{
		...
		public virtual void FaceVerticalDirection(Direction.Vertical _direction) //Called when the Controller is moved left or right, and thus might change direction; calls Turn() if we did change
		{
		...
		public virtual void Turn() //The visual component of a Turn; scales the X axis to positive or negative, and plays our slotted Turn AnimationClip
		{
			...
			if(slots.actor.gameObject.activeSelf)
			{
				StartCoroutine("TurnCoroutine");
			}
		}

		public void SetDirection(Direction.Horizontal _direction)
		{
			float scaleMultiplier = (_direction == Direction.Horizontal.Left) ? -1.0f : 1.0f;
			slots.actor.transform.localScale = new Vector3(scaleMultiplier, slots.actor.transform.localScale.y, slots.actor.transform.localScale.z);
		...
		{
			if(!isTurningAllowed || isKnockbackActive || (slots.actor.currentAttack != null && !slots.actor.currentAttack.actionsAllowedDuringAttack.turning))
			{
				return false;
			}
		...
		public void PlayDamagedSound()
		{
			...
			if(knockback)
			{
				slots.actor.PlaySoundIfOnCamera(knockback.audioClip);
			}
		}
		...
		public void PlaySingleAnimation(AnimationClip animation, bool _willAllowAttacks = false)
		{
			canAttackThroughOverridingAnimation = _willAllowAttacks;

			StopCoroutine("PlaySingleAnimationCoroutine");
			StartCoroutine("PlaySingleAnimationCoroutine", (object)animation);
		}

		public void PlayLoopingAnimation(AnimationClip animation, bool _willAllowAttacks = false)
		{
			canAttackThroughOverridingAnimation = _willAllowAttacks;
		...
		protected virtual void ApplyPhysicsObjectFlags() //Checks for Landing from a jump or fall, as well as a few other common physics things
		{
			...
			if(slots.physicsObject.DidLandThisFrame() || (StateID() == JumpState.idString && slots.physicsObject.IsOnSurface() && currentState.GetComponent<JumpState>().CanEnd()))
			{
				...
				if(jumpState != null)
				{
					jumpState.StopFrozenHorizontalMovement();
				}
		...
		public bool IsOverridingAnimationBlockingAttack()
		{
			return (isOverridingAnimationInProgress && !canAttackThroughOverridingAnimation);
		}
		...
		protected IEnumerator TurnCoroutine() //Turn() should be called rather than calling this directly
		{
			...
			isTurning = false;
			if(currentState && didPlayTurnAnimation)
			{
		...
		protected IEnumerator DisableOneWayPlatformsCoroutine() //Don't call this directly; this just helps DropThroughOneWayPlatform do its thing
		{
			if(slots.physicsObject)
			{
				...
				WallClingState wallClingState = GetComponent<WallClingState>();
				if(wallClingState != null)
				{
					wallClingState.ResetCooldownFrames();
				}
	...
	public class DeathState:RexState 
	{
		...
		public override void OnBegin()
		{
			if(controller.slots.physicsObject && controller.slots.actor.deathProperties.willFreezePhysics) 
			{
