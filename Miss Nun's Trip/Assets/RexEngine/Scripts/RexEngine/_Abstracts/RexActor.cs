...
namespace RexEngine
{
	...
	public class RexActor:RexObject
	{
		...
		public delegate void OnWeaponEnergyRestoredDelegate(int amount, string _energyTag, bool _requireActive);
		public event OnWeaponEnergyRestoredDelegate OnWeaponEnergyRestored;
		public event OnWeaponEnergyRestoredDelegate OnWeaponEnergyDecremented;

		public delegate void OnActorRevivedDelegate();
		public event OnActorRevivedDelegate OnActorRevived;
		...
		public class DeathProperties
		{
			...
			[Tooltip("If True, this actor's physics will freeze when it dies.")]
			public bool willFreezePhysics = true;
			[Tooltip("If True, this actor's HP and/or MP bars will be hidden as soon as it dies.")]
			public bool willHideEnergyBars;
			[Tooltip("If slotted, this AI Routine will play when the actor is killed.")]
			public RexAIRoutine deathAIRoutine;
		}
		...
		public class Invincibility
		{
			...
			[HideInInspector]
			public bool isAttackInvincibilityActive = false; //This is used to dampen damage from a single hit
		...
		{
			public bool disableForMovement;
			public bool disableForDialogue;
		}

		public enum DisableControlsType
		{
			Movement,
			Dialogue,
			Both
		...
		[HideInInspector]
		public DisableControls disableControls; //Lets you query if controls are disabled for movement and/or dialogue
		...
		void Awake()
		{
			...
			bool willDestroyEnergyBarsOnLoad = (gameObject.tag == "Player") ? false : true;
			if(hp)
			{
				hp.CreateBar(willDestroyEnergyBarsOnLoad);
			}

			if(mp)
			{
				mp.CreateBar(willDestroyEnergyBarsOnLoad);
			}
		}
		...
		public void StopBlink()
		{
			...
			for(int i = 0; i < len; i ++)
			{
				if(damagedProperties.spritesToFlash[i] != null)
				{
		...
		public void SetController(RexController _controller)
		{
			...
			_controller.SetStateToDefault(true, false, true);
			...
			if(slots.aiRoutine)
			{
				slots.aiRoutine.Disable();
				slots.aiRoutine.slots.controller = _controller;
			}

			if(_controller.slots.startingAIRoutine)
			{
				SetAIRoutine(_controller.slots.startingAIRoutine);
			}
		...
		//Swaps out the current AIRoutine for another
		public void SetAIRoutine(RexAIRoutine _routine)
		{
			RexAIRoutine previousRoutine = slots.aiRoutine;
			...
			_routine.gameObject.SetActive(true);
			_routine.Enable();

			slots.aiRoutine = _routine;
			slots.aiRoutine.slots.controller = slots.controller;

			OnAIRoutineChanged(_routine, previousRoutine);
		}
		...
		public void RemoveControl(DisableControlsType _type = DisableControlsType.Movement)
		{
			if(_type == DisableControlsType.Movement || _type == DisableControlsType.Both)
			{
				disableControls.disableForMovement = true;
			}
			else if(_type == DisableControlsType.Dialogue || _type == DisableControlsType.Both)
			{
				disableControls.disableForDialogue = true;
			}
		...
		public void RegainControl(DisableControlsType _type = DisableControlsType.Movement)
		{
			if(_type == DisableControlsType.Movement || _type == DisableControlsType.Both)
			{
				disableControls.disableForMovement = false;
			}
			else if(_type == DisableControlsType.Dialogue || _type == DisableControlsType.Both)
			{
				disableControls.disableForDialogue = false;
			}

			bool areControlsRemoved = (disableControls.disableForDialogue || disableControls.disableForMovement);

			if(!areControlsRemoved && slots.input)
			{
		...
		public void RestoreMP(int amount, string energyTag = "", bool requireActive = false)
		{
			if(mp != null && (mp.energyTag == energyTag || energyTag == "ALL"))
			{
				mp.Restore(amount);
			}

			if(OnWeaponEnergyRestored != null)
			{
				OnWeaponEnergyRestored(amount, energyTag, requireActive);
			}
		}

		//If an MP Energy component is slotted, subtract from the current value
		public void DecrementMP(int amount, string energyTag = "", bool requireActive = false)
		{
			if(mp != null && (mp.energyTag == energyTag || energyTag == "ALL"))
			{
				mp.Decrement(amount);
			...
			if(OnWeaponEnergyDecremented != null)
			{
				OnWeaponEnergyDecremented(amount, energyTag, requireActive);
			}
		}
		...
		public int Damage(int amount, bool willCauseKnockback = true, BattleEnums.DamageType damageType = BattleEnums.DamageType.Regular, Collider2D col = null) 
		{
			...
			if(slots.controller)
			{
				slots.controller.PlayDamagedSound();
			}
		...
		public void Revive() //Revives the actor from the Death state
		{
			...
			if(slots.physicsObject)
			{
				slots.physicsObject.ClearForces();
			}
			...
			if(slots.controller)
			{
				...
				if(mp != null)
				{
					if(mp.refillOnActorRevive.refillType == Energy.RefillType.Refill)
					{
						mp.SetToMax();
					}
					else if(mp.refillOnActorRevive.refillType == Energy.RefillType.LoseAll)
					{
						mp.Decrement(mp.max);
					}
					else if(mp.refillOnActorRevive.refillType == Energy.RefillType.SetToValue)
					{
						mp.SetValue(mp.refillOnActorRevive.setToValue);
					}
				}
			...
			if(deathProperties.willHideEnergyBars)
			{
				ShowEnergyBars();
			}
		...
		public void ForceDamagedStateEnd()
		{
		...
		protected void SetDeathParameters()
		{
			...
			if(slots.physicsObject)
			{
				if(deathProperties.willFreezePhysics)
				{
			...
			if(deathProperties.deathAIRoutine)
			{
				SetAIRoutine(deathProperties.deathAIRoutine);
			}
		...
		{
			return (invincibility.isAlwaysInvincible || invincibility.isDamagedInvincibilityActive || invincibility.isDebugInvincibilityActive || invincibility.isPowerupInvincibilityActive || invincibility.isAttackInvincibilityActive);
		}
		...
		protected virtual void OnAIRoutineChanged(RexAIRoutine _newRotuine, RexAIRoutine previousRoutine){} //This can be used to trigger specific effects when the ai controller changes
		...
		void OnDestroy()
		{
			...
			OnDamageTaken = null;
			OnWeaponEnergyRestored = null;
			OnWeaponEnergyDecremented = null;
		}
