...
namespace RexEngine
{
	public class ChargeAttack:AttackBase 
	{
			...
			if(Time.timeScale > 0.0f && slots.actor && isEnabled)
			{
				if(slots.actor.slots.input)
				{
					...
					else if(currentChargeTime > 0.0f)
					{
						...
						{
							if(currentChargeTime >= chargedAttacks[i].chargeTime && (slots.actor.mp == null || slots.actor.mp.current >= chargedAttacks[i].attack.energyCost))
							{
								currentAttack = i;
		...
		protected bool IsAttackAttempted()
		{
			...
			if(slots.actor.slots.input)
			{
				if((input.button == Attack.AttackImportance.Primary || input.button == Attack.AttackImportance.All) && slots.actor.slots.input.isAttackButtonDown)
				{
					isAttackAttempted = true;
				}
				else if((input.button == Attack.AttackImportance.Sub || input.button == Attack.AttackImportance.All) && slots.actor.slots.input.isSubAttackButtonDown)
				{
					isAttackAttempted = true;
				}
				else if((input.button == Attack.AttackImportance.Sub_2 || input.button == Attack.AttackImportance.All) && slots.actor.slots.input.isSubAttack_2ButtonDown)
				{
					isAttackAttempted = true;
				}
				else if((input.button == Attack.AttackImportance.Sub_3 || input.button == Attack.AttackImportance.All) && slots.actor.slots.input.isSubAttack_3ButtonDown)
				{
					isAttackAttempted = true;
				}
			}
