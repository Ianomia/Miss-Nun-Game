...
namespace RexEngine
{
	...
	public class Projectile:RexObject
	{
		...
		public class Reflection
		{
			...
			public bool willDestroyOnReflectorHit = false;
		...
		void Update()
		{
			...
			{
				float buffer = 2.5f;
				if(!CameraHelper.Instance.CameraContainsPoint(transform.position, buffer) && !isDead)
				{
					Clear();
				}
			}
		...
		public void Fire(Vector2 _startingPosition, Direction.Horizontal _horizontal, Direction.Vertical _vertical, RexActor _spawningActor, RexPool _parentSpawnPool = null)
		{
			...
			slots.spriteRenderer.transform.localEulerAngles = new Vector3(0.0f, 0.0f, 0.0f);
		...
		public void FireAtAngle(float angle, Vector2 _startingPosition, Direction.Horizontal _horizontal, RexActor _spawningActor, RexPool _parentSpawnPool = null)
		{
			if(!willDestroyWhenSceneChanges)
			{
				DontDestroyOnLoad(gameObject);
			...
			SetPosition(_startingPosition);
			SetHorizontalDirection(_horizontal);
			SetVerticalDirection((Direction.Vertical)(1 * PhysicsManager.Instance.gravityScale));
			...
			Vector2 angularVelocity = RexMath.VelocityForAngle(movementSpeed.x, angle);
			startingXSpeed = angularVelocity.x;
			startingYSpeed = angularVelocity.y;

			slots.physicsObject.SetVelocityX(startingXSpeed);
			slots.physicsObject.SetAccelerationCapX(movementSpeed.x);

			slots.physicsObject.SetVelocityY(startingYSpeed);
			slots.physicsObject.SetAccelerationCapY(movementSpeed.y);

			slots.spriteRenderer.transform.localEulerAngles = new Vector3(0.0f, 0.0f, angle);
		}

		public void FireTowardsPoint(Vector2 point, Vector2 _startingPosition, Direction.Horizontal _horizontal, RexActor _spawningActor, RexPool _parentSpawnPool = null)
		{
			float angle = RexMath.AngleFromPoint(new Vector3(_startingPosition.x, _startingPosition.y, 0.0f), point);
			FireAtAngle(angle, _startingPosition, _horizontal, _spawningActor, _parentSpawnPool);
		}
		...
		protected void ProcessCollision(Collider2D col)
		{
			...
			else if(col.gameObject.tag == "Reflector")
			{
				...
				else if(!reflection.willReflectOnReflectors && reflection.willDestroyOnReflectorHit)
				{
					OnDeath();
				}
			}
