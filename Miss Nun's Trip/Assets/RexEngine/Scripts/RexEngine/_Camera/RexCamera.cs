...
namespace RexEngine
{
	public class RexCamera:RexCameraBase 
	{
		...
		public class ScrollProperties
		{
			...
			[HideInInspector]
			public float backgroundOffset = 0.35f;

			[HideInInspector]
			public float midgroundOffset = 0.5f;

			[HideInInspector]
			public float foregroundOffset = 1.2f; 
		}
		...
		void Awake() 
		{
			...
			cameras.main.cullingMask = 1 << LayerMask.NameToLayer("Default") | 1 << LayerMask.NameToLayer("TransparentFX") | 1 << LayerMask.NameToLayer("Ignore Raycast") | 1 << LayerMask.NameToLayer("Water") | 1 << LayerMask.NameToLayer("Player") | 1 << LayerMask.NameToLayer("Terrain") | 1 << LayerMask.NameToLayer("PassThroughBottom") | 1 << LayerMask.NameToLayer("Stairs") | 1 << LayerMask.NameToLayer("Pushable");
		}
		...
		private void ScrollSecondaryCameras()
		{
			...
			{
				float foregroundX = (secondaryScrolling.willScrollForegroundX) ? transform.position.x * secondaryScrolling.foregroundOffset : transform.position.x;
				float foregroundY = (secondaryScrolling.willScrollForegroundY) ? transform.position.y * secondaryScrolling.foregroundOffset : transform.position.y;
			...
			{
				float midgroundX = (secondaryScrolling.willScrollMidgroundX) ? transform.position.x * secondaryScrolling.midgroundOffset : transform.position.x;
				float midgroundY = (secondaryScrolling.willScrollMidgroundY) ? transform.position.y * secondaryScrolling.midgroundOffset : transform.position.y;
			...
			{
				float backgroundX = (secondaryScrolling.willScrollBackgroundX) ? transform.position.x * secondaryScrolling.backgroundOffset : transform.position.x;
				float backgroundY = (secondaryScrolling.willScrollBackgroundY) ? transform.position.y * secondaryScrolling.backgroundOffset : transform.position.y;
