...
namespace RexEngine
{
	...
	{
		public int tiles = 5;
		[Tooltip("The amount of space between the bottom of the collider and the floor. This is mostly an edge-case and can safely be left alone in most circumstances. Raising this can be useful if things are colliding with the bottom of the ladder.")]
		public float colliderBottomMargin = 1.05f;
		...
		void Update()
		{
			if(!Application.isPlaying)
			{
				...
				if(previousTiles != tiles)
				{
					...
					float buffer = GlobalValues.tileSize * colliderBottomMargin;
					col.size = new Vector2(GlobalValues.tileSize, tiles * GlobalValues.tileSize);
					col.offset = new Vector2(0.0f, 0.0f);

					CreateSprites();

					col.size = new Vector2(GlobalValues.tileSize, (tiles * GlobalValues.tileSize) - buffer);
					col.offset = new Vector2(0.0f, buffer * 0.5f);
				}
