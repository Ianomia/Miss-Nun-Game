...
namespace RexEngine
{
	public class RexMenu:MonoBehaviour 
	{
		...
		private static void CreateBlankPlayer()
		{
            ...
            if(PrefabUtility.IsPartOfAnyPrefab(playerGameObject))
            {
                PrefabUtility.UnpackPrefabInstance(playerGameObject, PrefabUnpackMode.Completely, InteractionMode.AutomatedAction);
            }
#else
...
#if UNITY_2018_3_OR_NEWER
            if(PrefabUtility.IsPartOfAnyPrefab(enemyGameObject))
            {
                PrefabUtility.UnpackPrefabInstance(enemyGameObject, PrefabUnpackMode.Completely, InteractionMode.AutomatedAction);
            }
#else
...
#if UNITY_2018_3_OR_NEWER
            if(PrefabUtility.IsPartOfAnyPrefab(titleScriptGameObject))
            {
                PrefabUtility.UnpackPrefabInstance(titleScriptGameObject, PrefabUnpackMode.Completely, InteractionMode.AutomatedAction);
            }
#else
...
#if UNITY_2018_3_OR_NEWER
            if(PrefabUtility.IsPartOfAnyPrefab(logoGameObject))
            {
                PrefabUtility.UnpackPrefabInstance(logoGameObject, PrefabUnpackMode.Completely, InteractionMode.AutomatedAction);
            }
#else
...
#if UNITY_2018_3_OR_NEWER
            if(PrefabUtility.IsPartOfAnyPrefab(pressToStartGameObject))
            {
                PrefabUtility.UnpackPrefabInstance(pressToStartGameObject, PrefabUnpackMode.Completely, InteractionMode.AutomatedAction);
            }
#else
...
#if UNITY_2018_3_OR_NEWER
            if(PrefabUtility.IsPartOfAnyPrefab(madeWithRexGameObject))
            {
                PrefabUtility.UnpackPrefabInstance(madeWithRexGameObject, PrefabUnpackMode.Completely, InteractionMode.AutomatedAction);
            }
#else
		...
		[MenuItem("Tools/Rex Engine/Setup Minimal Actor", false, 300)]
		public static void SetupMinimalActor()
		{
			...
			if(actor != null)
			{
				Object prefab = Resources.Load("System/InputManager_Minimal") as Object;
				GameObject inputGameObject = Instantiate(prefab, SceneView.lastActiveSceneView.pivot, Quaternion.identity) as GameObject;
				inputGameObject.transform.parent = actor.transform;
				inputGameObject.name = "InputManager";
				inputGameObject.transform.localPosition = Vector3.zero;

				actor.damagedProperties.willUseHitSparks = false;
				actor.deathProperties.willNotifyGameManager = false;
				EditorUtility.SetDirty(actor);
			}
			else
			{
				EditorUtility.DisplayDialog("Oh no!", "Please select a GameObject with a RexActor component.", "Got it!");
			}
		}
...
#if UNITY_2018_3_OR_NEWER
                //PrefabUtility.UnpackPrefabInstance(meleeGameObject, PrefabUnpackMode.Completely, InteractionMode.AutomatedAction);
#else
...
#if UNITY_2018_3_OR_NEWER
                //PrefabUtility.UnpackPrefabInstance(projectileGameObject, PrefabUnpackMode.Completely, InteractionMode.AutomatedAction);
#else
...
#if UNITY_2018_3_OR_NEWER
                //PrefabUtility.UnpackPrefabInstance(attackSetGameObject, PrefabUnpackMode.Completely, InteractionMode.AutomatedAction);
#else
...
#if UNITY_2018_3_OR_NEWER
                //PrefabUtility.UnpackPrefabInstance(comboChainGameObject, PrefabUnpackMode.Completely, InteractionMode.AutomatedAction);
		...
		[MenuItem("Tools/Rex Engine/Add Ability/Add Pushing", false, 0)]
		public static void AddPushingState()
		{
			GameObject selectedObject = GetSelectGameObject();
			if(selectedObject)
			{
				if(selectedObject.GetComponent<PushingState>())
				{
					DisplayStateAlreadyExistsMessage("Pushing");
				}
				else
				{
					RexState state = selectedObject.AddComponent<PushingState>();
					state.controller = selectedObject.GetComponent<RexController>();
				}
			}
		}
		...
		[MenuItem("Tools/Rex Engine/Add AI", false, 0)]
		public static void AddAI()
		{
			GameObject selectedObject = Selection.activeGameObject;
			if(selectedObject)
			{
				RexActor rexActor = selectedObject.GetComponent<RexActor>();
				RexController rexController = null;

				if(rexActor)
				{
					rexController = rexActor.slots.controller;
					if(rexController == null)
					{
						rexController = rexActor.GetComponentInChildren<RexController>();
						if(rexController != null)
						{
							rexActor.slots.controller = rexController;
						}
					}

					if(rexActor && rexController)
					{
						GameObject actorGameObject = rexActor.gameObject;

						Transform aiTransform = actorGameObject.transform.Find("AI");
						GameObject aiGameObject = null;
						if(aiTransform != null)
						{
							aiGameObject = aiTransform.gameObject;
						}

						if(aiGameObject == null)
						{
							aiGameObject = new GameObject();
							aiGameObject.name = "AI";
							aiGameObject.transform.parent = actorGameObject.transform;
							aiGameObject.transform.localScale = Vector3.one;
							aiGameObject.transform.localPosition = Vector3.zero;
						}

						GameObject aiRoutineGameObject = new GameObject();
						aiRoutineGameObject.name = "AI_Routine";
						aiRoutineGameObject.transform.parent = aiGameObject.transform;
						aiRoutineGameObject.transform.localScale = Vector3.one;
						aiRoutineGameObject.transform.localPosition = Vector3.zero;

						RexAIRoutine rexAIRoutine = aiRoutineGameObject.AddComponent<RexAIRoutine>();
						rexAIRoutine.slots = new RexAIRoutine.Slots();
						rexAIRoutine.slots.controller = rexActor.slots.controller;

						if(rexActor.slots.aiRoutine == null)
						{
							rexActor.slots.aiRoutine = rexAIRoutine;
						}

						if(rexController.slots.startingAIRoutine == null)
						{
							rexController.slots.startingAIRoutine = rexAIRoutine;
						}
					}
					else
					{
						EditorUtility.DisplayDialog("Oh no!", "Please ensure that your RexActor has a RexController component slotted first.", "Got it!");
					}
				}
				else 
				{
					EditorUtility.DisplayDialog("Oh no!", "Please select a RexActor.", "Got it!");
				}
			}
			else 
			{
				EditorUtility.DisplayDialog("Oh no!", "Please select a RexActor first.", "Got it!");
			}
		}
		...
		public static void FillRexActorSlots(bool willDisplayPopupDialogue = true)
		{
			...
			if(actor != null)
			{
				...
				actor.slots.aiRoutine = actor.GetComponentInChildren<RexAIRoutine>();

				if(actor.slots.spriteHolder != null)
				{
				...
				if(controller)
				{
					...
					if(actor.slots.aiRoutine)
					{
						controller.slots.startingAIRoutine = actor.slots.aiRoutine;
					}
				}
