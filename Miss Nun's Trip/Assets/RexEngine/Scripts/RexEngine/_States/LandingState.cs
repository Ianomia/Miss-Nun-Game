	...
	public class LandingState:RexState
	{
		...
		public void CheckStun(float distanceFallen)
		{
			...
			if(willStun)
			{
				...
				controller.stunDuration = (animationClip != null ) ? animationClip.length : 0.0f;
			}
		}
		...
		protected void SpawnLandingParticle()
		{
			...
			int verticalDirection = (controller.GravityScaleMultiplier() >= 0) ? 1 : -1;
			particle.transform.localScale = new Vector3((int)controller.direction.horizontal, verticalDirection, 0.0f);
