...
namespace RexEngine
{
	public class BounceState:RexState 
	{
		...
		public override void UpdateMovement()
		{
			...
			{
				currentBounceFrame ++;
				if((currentBounceFrame >= minFrames && !controller.slots.input.isJumpButtonDown) || currentBounceFrame >= maxFrames) //End the bounce
				{
					if(currentHangtimeFrame < hangtimeFrames)
					{
						controller.slots.physicsObject.SetVelocityY(0.0f);
						CheckHangtimeFrames();
					}
				...
				{
					controller.slots.physicsObject.ApplySingleFrameVelocityAddition(new Vector2(0.0f, speed * controller.GravityScaleMultiplier()));
				}
			}
		}
