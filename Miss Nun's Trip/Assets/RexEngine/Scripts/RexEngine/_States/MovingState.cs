...
namespace RexEngine
{
	public class MovingState:RexState
	{
		...
		protected void ApplyMovement(float _inputDirection, MovementAxis movementAxis)
		{
			...
			bool isLockedIntoJump = (jumpState && (controller.StateID() == BaseJumpState.idString || controller.StateID() == FallingState.idString) && jumpState.IsHorizontalMovementFrozen());
			if(isLockedIntoJump)
			{
			...
			else //We can't move directly, but we can apply residual deceleration
			{
				if(movementAxis == MovementAxis.Horizontal)
				{
					...
					if(Mathf.Abs(controller.slots.physicsObject.properties.velocity.x) <= 0.0f && (controller.slots.physicsObject.IsOnSurface() || canMoveVertically))
					{
						if(!hasEnded)
						{
							if(!controller.isKnockbackActive && controller.StateID() != LadderState.idString && controller.StateID() != StairClimbingState.idString && controller.StateID() != PushingState.idString && controller.StateID() != LandingState.idString)
							{
		...
		{
			if(((controller.slots.physicsObject.IsOnSurface() && !canMoveVertically) || (canMoveVertically && movementAxis == MovementAxis.Horizontal)) && controller.StateID() != id && controller.currentState.id != JumpState.idString && controller.currentState.id != LadderState.idString && controller.currentState.id != CrouchState.idString && controller.currentState.id != PushingState.idString)
			{
				return true;
			}
			else
