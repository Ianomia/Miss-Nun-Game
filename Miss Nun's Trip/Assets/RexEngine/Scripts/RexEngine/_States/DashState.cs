...
namespace RexEngine
{
	public class DashState:RexState 
	{
		...
		[Tooltip("If True, the actor's horizontal acceleration will be stopped when the dash starts and the dash will immediately go to top speed. Recommended.")]
		public bool killAccelerationOnStart = true;
		...
		void Update()
		{
			if(isEnabled && !IsFrozen() && !controller.isKnockbackActive)
			{
				...
				bool isDashAttempted = controller.slots.input && controller.slots.input.isDashButtonDownThisFrame;
		...
		public override void OnBegin()
		{
			...
			controller.SetAxis(new Vector2(dashStartDirection.x, dashStartDirection.y));
		...
		protected void SetDashStartDirection()
		{
			...
			float horizontalAxis = (controller.slots.input) ? controller.slots.input.horizontalAxis : (float)controller.direction.horizontal;
			float verticalAxis = (controller.slots.input) ? controller.slots.input.verticalAxis : 0.0f;

			bool isDashingDiagonally = false;

			if(verticalAxis == 1.0f && Mathf.Abs(horizontalAxis) == 1.0f && dashDirectionsAllowed.diagonalUp)
			{
			...
			else if(verticalAxis == -1.0f && Mathf.Abs(horizontalAxis) == 1.0f && dashDirectionsAllowed.diagonalDown)
			{
				isDashingDiagonally = true;
				dashStartDirection.x = (int)controller.direction.horizontal;
				dashStartDirection.y = -1.0f * controller.GravityScaleMultiplier();
			}
			else if(verticalAxis == 1.0f && dashDirectionsAllowed.up)
			{
				dashStartDirection.y = 1.0f * controller.GravityScaleMultiplier();
			}
			else if(verticalAxis == -1.0f && dashDirectionsAllowed.down)
			{
				dashStartDirection.y = -1.0f * controller.GravityScaleMultiplier();
			}
