...
namespace RexEngine
{
	public class JumpState:BaseJumpState
	{
		...
		void Update() 
		{
			...
			{
				CheckJumpBuffer();
				bool isJumpAttempted = controller.slots.input && controller.slots.input.isJumpButtonDownThisFrame || (isJumpBuffered && controller.slots.physicsObject.IsOnSurface());
		...
		public override bool CanInitiate()
		{
            ...
            else if(currentBlockForFrames > 0)
            {
                canInitiateJump = false;
            }
		...
		public override void OnBegin()
		{
			...
			controller.slots.physicsObject.ApplySingleFrameVelocityAddition(new Vector2(0.0f, adjustedSpeed * controller.GravityScaleMultiplier()));
		...
		public void StopFrozenHorizontalMovement()
		{
			if(freezeHorizontalMovement)
			{
				if(controller.slots.input)
				{
					direction = Direction.Horizontal.Neutral;
				}
			}
		}
		...
		protected override void ResetFlags()
		{
            ...
            if(currentBlockForFrames > 0)
            {
                currentBlockForFrames--;
            }
			...
			if((isOnSurface && !isJumpActive) || (controller.StateID() == WallClingState.idString && wallClingState != null && wallClingState.IsWallJumpPossible()))
			{
				currentJump = 0;
				currentGraceFrame = 0;
			}
