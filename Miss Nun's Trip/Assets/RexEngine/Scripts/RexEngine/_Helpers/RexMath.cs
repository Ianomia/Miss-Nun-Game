...
public class RexMath:MonoBehaviour 
{
	...
	public static Vector2 VelocityForAngle(float speed, float angle)
	{
		Vector2 speedVector = Vector2.zero;
		float adjustedAngle = angle / 57.297469f;

		speedVector.x = speed * Mathf.Cos(adjustedAngle);
		speedVector.y = speed * Mathf.Sin(adjustedAngle);
