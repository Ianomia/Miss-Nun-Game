...
namespace RexEngine
{
	public class GameManager:MonoBehaviour 
	{
		...
		public class CameraSettings
		{
			...
			public Vector2 roomSize = new Vector2(25.8f, 14.55f);
		}
		...
		public void MakePlayersActive()
		{
			...
			for(int i = 0; i < GameManager.Instance.players.Count; i ++)
			{
				...
				player.disableControls.disableForDialogue = false;
				player.disableControls.disableForMovement = false;
		...
		protected IEnumerator PlayerDeathCoroutine()
		{
			...
			for(int i = 0; i < players.Count; i ++)
			{
				...
				if(!players[i].isDead)
				{
					...
					if(additionalPlayer.deathProperties.willFreezePhysics)
					{
			...
			{
				string sceneToLoad = LivesManager.Instance.lastSavedScene;
				RexSceneManager.Instance.LoadSceneWithFadeOut(sceneToLoad, Color.white, false, false);

				for(int i = 0; i < players.Count; i ++)
				{
					RexActor additionalPlayer = players[i];
					additionalPlayer.RegainControl(RexActor.DisableControlsType.Both);
				}
