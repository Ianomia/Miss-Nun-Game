...
namespace RexEngine
{
	public class RexSceneManager:MonoBehaviour 
	{
		...
		public void LoadSceneWithFadeOut(string level, Color fadeColor = default(Color), bool willStartFreshFade = true, bool _willNextSceneFadeIn = true, ScreenFade.FadeDuration _duration = ScreenFade.FadeDuration.Medium)
		{
			StopAllCoroutines();
			...
			isLoadingNewScene = true;
			PurgeSceneLoaders();
			ScreenFade.Instance.Fade(ScreenFade.FadeType.Out, _duration, fadeColor, willStartFreshFade);
		...
		public void DestroyScene()
		{
			...
			//Energy bars need to be cleared separately, since they auto-parent themselves to the UI GameObject which is in DontDestroyOnLoad
			GameObject ui = GameObject.Find("UI");
			if(ui)
			{
				foreach(EnergyBar energyBar in ui.GetComponentsInChildren<EnergyBar>())
				{
					if(energyBar.willDestroyWhenSceneChanges)
					{
