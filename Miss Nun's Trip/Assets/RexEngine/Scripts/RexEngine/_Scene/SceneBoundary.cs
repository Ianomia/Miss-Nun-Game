...
namespace RexEngine
{
	...
	public class SceneBoundary:MonoBehaviour
	{
		...
		private RexSettingsData rexSettingsData;
		...
		protected void SnapToRoomSize()
		{
			...
			if(rexSettingsData == null)
			{
				...
				foreach(string guid in results)
				{
					...
					rexSettingsData = UnityEditor.AssetDatabase.LoadAssetAtPath(path, typeof(RexSettingsData)) as RexSettingsData;
				}
			}

			Vector2 roomSize = rexSettingsData.cameraSettings.roomSize;

			if(edge == Edge.Right)
			{
				roomPosition = Mathf.CeilToInt((transform.position.x - (roomSize.x * 0.5f)) / roomSize.x);
				if(roomPosition < 0)
				{
					roomPosition = 0;
				}

				transform.position = new Vector3((roomPosition * roomSize.x) + (roomSize.x * 0.5f), transform.position.y, transform.position.z);
			}
			else if(edge == Edge.Left)
			{
				roomPosition = Mathf.FloorToInt((transform.position.x + (roomSize.x * 0.5f)) / roomSize.x);
				if(roomPosition > 0)
				{
					roomPosition = 0;
				}
			...
			{
				roomPosition = Mathf.FloorToInt((transform.position.y - (roomSize.y * 0.5f)) / roomSize.y);
				if(roomPosition < 0)
				{
					roomPosition = 0;
				}
				transform.position = new Vector3(transform.position.x, (roomPosition * roomSize.y) + (roomSize.y * 0.5f), transform.position.z);
			}
			else if(edge == Edge.Bottom)
			{
				roomPosition = Mathf.CeilToInt((transform.position.y + (roomSize.y * 0.5f)) / roomSize.y);
				if(roomPosition > 0)
				{
					roomPosition = 0;
				}
