...
namespace RexEngine
{
	public class SceneLoader:MonoBehaviour 
	{
		...
		public ScreenFade.FadeDuration fadeDuration = ScreenFade.FadeDuration.Medium;
		...
		public void LoadSceneWithFadeOut()
		{
			if(!hasTriggeredLoad)
			{
				...
				RexSceneManager.Instance.LoadSceneWithFadeOut(levelToLoad, fadeColor, true, true, fadeDuration);
			}
		}
	...
	public class SceneLoaderEditor:Editor
	{
		...
		public override void OnInspectorGUI()
		{
			...
			SceneLoader _target = target as SceneLoader;

			EditorGUILayout.BeginVertical(EditorStyles.helpBox);

			EditorGUILayout.LabelField("Load Connected Scene", EditorStyles.boldLabel);

			EditorGUILayout.LabelField("Loads the connected scene in the Editor. If the scene is in a different folder from the currently open scene, you may have to manually populate the below path to the connected scene's folder.", styles.helpTextStyle);

			_target.newSceneFolderPath = EditorGUILayout.TextField("New Scene Folder Path", _target.newSceneFolderPath);
			if(_target.newSceneFolderPath == "" || _target.newSceneFolderPath == null)
			{
				string thisScenePath = EditorSceneManager.GetActiveScene().path;
				string[] thisScenePathArray = thisScenePath.Split('/');
				thisScenePath = "";

				for(int i = 0; i < thisScenePathArray.Length - 1; i ++)
				{
					thisScenePath += thisScenePathArray[i] + '/';
				}

				_target.newSceneFolderPath = thisScenePath;
			}

			if(GUILayout.Button("Load Scene"))
			{
				string sceneToLoad = _target.newSceneFolderPath + _target.levelToLoad + ".unity";
