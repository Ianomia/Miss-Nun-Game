...
namespace RexEngine
{
	public class RexInput:MonoBehaviour
	{
		...
		[HideInInspector]
		public bool isDashButtonDownThisFrame = false;
		[HideInInspector]
		...
		public void ClearInput()
		{
			...
			isDashButtonDownThisFrame = false;
			isMisc_1ButtonDown = false;
			isMisc_1ButtonDownThisFrame = false;
			isMisc_2ButtonDown = false;
			isMisc_2ButtonDownThisFrame = false;
			isRunButtonDown = false;
			isAdvanceTextButtonDown = false;
		}
		...
		protected virtual void SetInputs()
		{
			if(isEnabled && Time.timeScale > 0 || willAcceptInputWhenPaused)
			{
				...
				isDashButtonDown = InputManager.Instance.GetButton(playerId, InputAction.Dash);
				isDashButtonDownThisFrame = InputManager.Instance.GetButtonDown(playerId, InputAction.Dash);
